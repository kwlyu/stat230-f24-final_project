...
commute_v_gender <- ggplot(happiness_recode, aes(x = commute, y = happy, color = gender)) +
  geom_jitter(width = 0.2, height = 0.2, alpha = 0.3) +
  labs(y = "Happy", x = "Commute Time", color = "Gender") +
  scale_y_continuous(breaks = c(0, 1))
edu_v_gender <- ggplot(happiness_recode, aes(x = commute, y = happy, color = gender)) +
  geom_jitter(width = 0.2, height = 0.2, alpha = 0.3) +
...
(empirical_log_odds_plot$commute_gender | empirical_log_odds_plot$educ_gender) / (commute_v_gender | edu_v_gender)
...
```{r}
#EDA implies possible interaction between gender and education,
    #and gender and commute time
...
happinessBase_aug<-augment(happiness_glm, resid.type="pearson")
  #Commute
happinessBase_aug$CommuteGroups<-ntile(happinessBase_aug$commute, n=75)
happinessBase_aug_resid1<- happinessBase_aug %>%
  group_by(CommuteGroups) %>%
    summarize(CommuteGroupsMed = median(commute),
              resid_mean=mean(.resid), resid_med=median(.resid))
  #Education
happinessBase_aug$EducGroups<-ntile(happinessBase_aug$educ, n=75)
happinessBase_aug_resid2<- happinessBase_aug %>%
  group_by(EducGroups) %>%
    summarize(EducGroupsMed = median(educ),
              resid_mean=mean(.resid), resid_med=median(.resid))
  #Income
happinessBase_aug$YGroups<-ntile(happinessBase_aug$realrinc, n=75)
happinessBase_aug_resid3<- happinessBase_aug %>%
  group_by(YGroups) %>%
    summarize(YGroupsMed = median(realrinc),
              resid_mean=mean(.resid), resid_med=median(.resid))

#Assumption Checking: Residual Plots
  #Commute Time
ggplot(data=happinessBase_aug_resid1, aes(x=CommuteGroups, y=resid_mean))+
  geom_point()+
  ...
  #Education
ggplot(data=happinessBase_aug_resid2, aes(x=EducGroups, y=resid_mean))+
  geom_point()+
  geom_hline(yintercept=c(-2, 0, 2), linetype="dotted")+
  labs(x="Grouped Education levels", y="Mean Pearson Residual", 
       title="Residual Plot of Grouped Education Level")
  #Income
ggplot(data=happinessBase_aug_resid3, aes(x=YGroups, y=resid_mean))+
  geom_point()+
  geom_hline(yintercept=c(-2, 0, 2), linetype="dotted")+
...
happinessBase_aug <- happinessBase_aug %>% mutate(case = row_number())
    #Check Cook's Distances
ggplot(happinessBase_aug, aes(x=case, y=.cooksd))+
  geom_point()+
...
ggplot(happinessBase_aug, aes(x=case, y=.std.resid))+
  geom_point()+
  ...
  #Examine Leverage
ggplot(happinessBase_aug, aes(x=case, y=.hat))+
  geom_point()+
...
which(happinessBase_aug$.hat>6/874)
